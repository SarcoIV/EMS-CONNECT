...
import{r as a,j as y,_}from"./app-yYWzFYAY.js";/* empty css            */const b={medical:"🏥",fire:"🔥",accident:"🚗",crime:"🚨",natural_disaster:"🌊",other:"⚠️"};function L({incident:r,selectedResponder:t}){const f=a.useRef(null),e=a.useRef(null),h=a.useRef(null),s=a.useRef(null),c=a.useRef(null),[p,m]=a.useState(!1);return a.useEffect(()=>{const u=document.createElement("link");return u.rel="stylesheet",u.href="https://cdnjs.cloudflare.com/ajax/libs/leaflet/1.9.4/leaflet.min.css",document.head.appendChild(u),()=>{document.head.removeChild(u)}},[]),a.useEffect(()=>((async()=>{if(!(!f.current||p))try{const n=(await _(async()=>{const{default:d}=await import("./leaflet-src-BF5mfi_M.js").then(g=>g.l);return{default:d}},__vite__mapDeps([0,1,2]))).default,o=n.map(f.current).setView([r.latitude,r.longitude],13);n.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{attribution:"&copy; OpenStreetMap contributors",maxZoom:19}).addTo(o);const i=n.divIcon({className:"custom-div-icon",html:`
                ...
                `),e.current=o,h.current=l,m(!0)}catch(n){console.error("Failed to load map:",n)}})(),()=>{e.current&&(e.current.remove(),e.current=null,m(!1))}),[r]),a.useEffect(()=>{if(!p||!e.current)return;(async()=>{const n=(await _(async()=>{const{default:o}=await import("./leaflet-src-BF5mfi_M.js").then(i=>i.l);return{default:o}},__vite__mapDeps([0,1,2]))).default;if(s.current&&(e.current.removeLayer(s.current),s.current=null),c.current&&(e.current.removeLayer(c.current),c.current=null),t){const o=t.current_latitude??t.base_latitude,i=t.current_longitude??t.base_longitude;if(o&&i){const l=n.divIcon({className:"custom-div-icon",html:`
                            <div style="
